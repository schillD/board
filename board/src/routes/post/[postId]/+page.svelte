<script lang="ts">
  ...
  else {alert("CurrentUser ist Null")}
  }
   ...
   <FormGroup >
    <h3>Comment: </h3>
     <div class="form-floating">
       <textarea class="form-control" placeholder="Body" bind:value={comment_text} id="floatingTextarea2" style="height: 100px"></textarea>
...
{#each comment_list as comment (comment.id)}
    ...
    <br />
    <p><a href={"/profile/" + comment.expand.user.id}>Author: {comment.expand.user.username}</a></p>
    </div>
{/each}
